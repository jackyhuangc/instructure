    ...
    <div class="layui-form">
      <div class="layui-form-item">
        <div class="layui-inline">
          <label class="layui-form-label">编号|名称|类型</label>
  ...
  methods: {
    ...
    initPage() {
      ...
      layui.use(["layer", "table", "form", "laypage", "laydate"], function() {
        ...
        vm.$http
          .post("http://" + API_SERVER + "/strive/product/query", {
            page: 1,
            size: 20,
            productNo: $("#content").val()
          })
          .then(rep => {
            var result = rep.body;
            if (result.code !== 0) {
      ...
      if (data) {
          ...
          cols: [
            [
              ...
              },
              {
                ...
                title: "促销类型",
                minWidth: 150,
                templet: "#promotionTpl",
                unresize: true
              },
              {
                field: "productPromotionBegin",
                title: "促销开始",
                minWidth: 200
              },
              {
                field: "productPromotionEnd",
                title: "促销结束",
                minWidth: 200
              },
              {
        ...
        vm.table.on("tool(tableEvent)", function(obj) {
          ...
          if (obj.event === "edit") {
            ...
            layer.confirm(
              "Are you sure to delete [" + obj.data.productNo + "]？",
              function(index) {
                vm.$http
                  .post(
                    "http://" +
                      API_SERVER +
                      "/strive/product/delete/" +
                      obj.data.productNo
                  )
                  .then(rep => {
                    var result = rep.body;
